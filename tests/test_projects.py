...
from apps.projects.models import Comment, Project, Tag
...
class TestProjectService:
    def test_create_project(self, user, tag):
        project = ProjectService.create_project(
            owner=user,
            data={"title": "My Project", "description": "Desc", "visibility": "public", "status": "draft"},
            tag_names=["python"],
        )
    ...
    def test_get_visible_projects_public(self, user):
        ProjectService.create_project(
            owner=user,
            data={"title": "Public", "description": "d", "visibility": "public", "status": "published"},
        )
    ...
    def test_get_visible_projects_private_hidden(self, user):
        ProjectService.create_project(
            owner=user,
            data={"title": "Private", "description": "d", "visibility": "private", "status": "published"},
        )
        projects = ProjectService.get_visible_projects()
...
class TestCommentAPI:
    def test_list_comments(self, authenticated_client, project, user):
        ...
        response = authenticated_client.get(
            f"/api/v1/comments/?project={project.id}"
        )
        assert response.status_code == status.HTTP_200_OK
