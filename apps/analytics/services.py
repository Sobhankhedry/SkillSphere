...
class AnalyticsService:
    ...
    def get_user_dashboard(user):
        from apps.projects.models import Project, Comment, ProjectFile
        from apps.activity_logs.models import ActivityLog

        projects = Project.objects.filter(owner=user)
        return {
            "total_projects": projects.count(),
            "total_downloads": projects.aggregate(total=Count("download_count"))["total"] or 0,
            "total_comments": Comment.objects.filter(project__owner=user).count(),
            "recent_activities": list(
                ActivityLog.objects.filter(user=user)
                .values("activity_type", "description", "created_at")[:10]
            ),
        }
    ...
    def get_admin_dashboard():
        ...
        return {
            ...
            "total_downloads": Project.objects.aggregate(
