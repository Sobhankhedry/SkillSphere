...
from .services import CommentService, FileService, ProjectService
...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [permissions.IsAuthenticatedOrReadOnly, IsOwnerOrCollaboratorOrReadOnly]
    ...
    def collaborating(self, request):
        ...
        projects = Project.objects.filter(id__in=project_ids).select_related("owner").prefetch_related("tags")
        page = self.paginate_queryset(projects)
        if page is not None:
            serializer = self.get_serializer(page, many=True)
            return self.get_paginated_response(serializer.data)
